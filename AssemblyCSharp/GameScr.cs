...
public class GameScr : mScreen, IChatable
{
	...
	public override void updateKey()
	{
		...
		else
		{
			...
			{
                CommandKey.Autorun(GameCanvas.keyAsciiPress);
                if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
	...
	{
		Func.Gi().updateGame();
        if (GameCanvas.gameTick % 10 == 0 && linhtinh.gI().isDapdo)
        {
            linhtinh.gI().dapdox();
        }
	...
	public override void paint(mGraphics g)
	{
		...
		for (int i = 0;i< this.BossVip.size(); i++)
		{
			...
			g.fillRect(GameCanvas.w - 23, num1111 + 2, 25, 9);
			((boss)this.BossVip.elementAt(i)).paintboss(g,GameCanvas.w-2, num1111, mFont.RIGHT);
            num1111 += 10;
        }
		resetTranslate(g);
		paint_xp_bar(g);
		...
		{
			g.setColor(UnityEngine.Color.red);// nhìn vth chứ k có màu
			for (int i = 0; i < vCharInMap.size(); i++)
			{
				Char nhanhvat = (Char)vCharInMap.elementAt(i);
				if (nhanhvat.cTypePk == 5)
				{
					g.drawLine(Char.myCharz().cx - cmx,Char.myCharz().cy - cmy, nhanhvat.cx - cmx, nhanhvat.cy - cmy);
					if (Func.Gi().checkboos == false) Func.Gi().checkboos = true;
                }
			}
		}
		if (!isPaintOther)
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 12006:
			...
			GameCanvas.endDlg();
			break;
		default: // gắn acctions cho bên kia // quang
				Func.Gi().acctions(idAction);
				break;
		}
	}
    ...
    public void chatVip(string chatVip)
	{
		...
		{
			this.BossVip.addElement(new boss(chatVip));
			if (BossVip.size() > 7)
			{
