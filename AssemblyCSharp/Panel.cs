...
public class Panel : IActionListener, IChatable
{
	...
	private void paintShop(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item != null)
			{
				...
				//id item shop
                mFont2.drawString(g, string.Concat(new object[]
				{
                    "[",item.template.id,"] ",item.template.name + text
                }) , num2 + 5, num3 + 1, 0);
				string text2 = string.Empty;
	...
	public void update()
	{
		if (CommandKey.Autorun(GameCanvas.keyAsciiPress))
		{
		}
		if (chatTField != null && chatTField.isShow)
		{
	...
	private void doFireShop()
	{
		...
		if (currentTabIndex < currentTabName.Length - ((GameCanvas.panel2 == null) ? 1 : 0) && type != 17)
		{
			...
			{
				//mua item
                myVector.addElement(new Command(mResources.BUYS , this, 02082003, currItem));
	...
	public static void muanhieu(Item item)
	{
		new Thread(delegate ()
		{
			for (int i = 0; i < linhtinh.solanmua; i++)
			{
                Service.gI().buyItem(0, (int)item.template.id, 0);
                Thread.Sleep(50);
            }
	...
	{
		if (idAction == 02082003)
		{
			Item item = (Item)p;
			linhtinh.ismuado = !linhtinh.ismuado;
			if (linhtinh.ismuado)
			{
				muanhieu(item);
            }
